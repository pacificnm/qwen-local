...
# per-run LLM calls: the old hand-rolled loop capped at 3 tool rounds; 8
# total calls covers list→read→write→commit plus the final answer while a
# stuck model cannot loop forever on a busy V100.
os.environ.setdefault("QWEN_AGENT_MAX_LLM_CALL_PER_RUN", "8")
